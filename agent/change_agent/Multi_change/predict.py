...
class Change_Perception(object):
    def define_args(self):
        ...
        parser.add_argument(
            "--data_folder", default="data/", help="folder with data files"
        )
        parser.add_argument(
            "--list_path",
            default=os.path.join(script_dir, "data", "LEVIR_MCI") + "/",
            help="path of the data lists",
        )
        parser.add_argument(
            "--vocab_file", default="vocab", help="path of the data lists"
        )
        parser.add_argument(
        ...
        # inference
        parser.add_argument(
            "--gpu_id", type=int, default=0, help="gpu id in the training."
        )
        parser.add_argument(
            "--checkpoint",
            default=os.path.join(parent_dir, "models_ckpt", "MCI_model.pth"),
            help="path to checkpoint",
        )
        parser.add_argument(
            "--result_path",
            default=os.path.join(script_dir, "predict_result") + "/",
        ...
        parser.add_argument(
            ...
            type=int,
            default=3,
            help="Number of layers in AttentionEncoder.",
        )
    ...
    def __init__(
        ...
        checkpoint = torch.load(snapshot_full_path, map_location=self.device)
        ...
        for obj_idx, obj_name in [(1, "road"), (2, "building")]:
            ...
            for prop in props:
                if prop.area > 5:
                    ...
                    prior = "area"
                    if "forest" in caption:
                        prior = "forest"
                    elif "bare" in caption:
                        prior = "bare land"
                    elif "grass" in caption:
                        prior = "grass"
                    elif "water" in caption:
                        prior = "water"
                    elif "farmland" in caption:
                        prior = "farmland"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mask_save_path", default=os.path.join(script_dir, "CDmask.png")
    )

...
async def chat(query: str = Form(...), images: List[UploadFile] = File(default=[])):
    ...
    if images and len(images) > 0:
        for image in images:
            if not image.filename:
                continue
            file_extension = (
                image.filename.split(".")[-1] if "." in image.filename else "png"
            )
            base_name = os.path.splitext(image.filename)[0] if image.filename else "image"
            unique_filename = f"{uuid.uuid4().hex[:8]}_{base_name}.{file_extension}"
    ...
    response = run_agent(
        query, image_paths=SESSION_STORE["images"], history=SESSION_STORE["history"]
    )
